...
class unstructShape:
    ...
    def __init__(self,points,old1p,old2p,old1c,old2c):
    ...
    def __add__(self,other):
        totpoints = self.points + other.points
		
		#SUBSTITUTE THIS WITH AN ALGORITHM THAT LOOKS FOR POINTS SUFFICIENTLY CLOSE, NOT PRECISELY COINCIDENT
        del_points  = list(set(totpoints))
        
		new_points  = sort(del_points)
		
		totconn = self.connections + other.connections 
        
        out         = unstructShape(new_points, totpoints, totconn, old1p = self.points, old2p = other.points, old1c = self.connections, old2c = other.connections)
        return out
